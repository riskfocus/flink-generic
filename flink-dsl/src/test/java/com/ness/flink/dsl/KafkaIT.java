...
import com.ness.flink.dsl.test.TestEventLong;
...
/**
 * Simple integration test, requiring Kafka up and running with docker-compose.yml provided. Demonstrates, how to write
 * relatively simple pipeline with two Kafka sources, connected to one operator, producing results to Kafka sink.
 */
@Slf4j
public class KafkaIT {
    ...
    void shouldReadWriteToKafka() {
        ParameterTool params = ParameterTool.fromMap(
            Map.of(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, "localhost:19092"));
        ...
        JobClient jobClient = StreamBuilder.from(env, params)
            .newDataStream()
    ...
    private static class TestEventStringProcessor extends
        KeyedProcessFunction<String, TestEventString, TestEventString> {

        @Override
        public void processElement(TestEventString value, Context ctx, Collector<TestEventString> out)
            throws Exception {
            log.info("Received: value={}", value);
            out.collect(value);
        }
